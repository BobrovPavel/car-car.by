...
**Этапы 1-5 закрыты: kufar, onliner и av.by собираются, мультиисточник в UI работает.
Сейчас — этап 6: парсер abw.by (ловим реальный API площадки).
Деплой (этап 7) ждёт capacity на Oracle для VPS. Параллельно полируем сайт локально.**
...
- `cars.db` — kufar (собирает `collector.py`)
- `cars_avby.db` — av.by (собирает `av_listing.py`)
- `cars_abw.db` — abw.by (планируется, этап 6)
...
cars_all = main.cars      ('kufar' AS source)
    ...
    [UNION ALL abw.cars    ('abw'  AS source)]   ← добавится на этапе 6
```
...
Колонка `source` в `db.py` не заложена; `av_listing.py` добавляет её в свою
...
## Этап 6. Парсер onliner.by 🔧 (в работе)
 
Цель: по образцу av.by сделать `onliner.py` → своя база `cars_onliner.db`
(схема `db.py`), `source='onliner'`, и расширить UNION в `web/api.py`.
 
**Сейчас на шаге разведки API.** abw.by — SPA за анти-ботом: прямой fetch и сам
SSR-сайт блокируются bot-detection, в поиске/на GitHub готового разбора нет.
По образцу av.by нужно **поймать реальные сетевые запросы из SPA** (DevTools →
Network → Fetch/XHR) и от их формы строить парсер.
 
**Что ловим (DevTools, Copy as cURL + кусок JSON-ответа):**
1. Запрос **листинга** (массив объявлений) — `https://ab.onliner.by/sdapi/ab.api/search/vehicles`.
2. **Пагинация** — тот же запрос для 2-й страницы (параметры: page и limit).
3. **Карточка** одного объявления — `get(f"{VEHICLE_URL}/{ad_id}"` берём отсюда description 
4. Запрос **VIN** `f"{VEHICLE_URL}/{ad_id}/vin"`

 
---
 ## Этап 7. Парсер abw.by 🔧 

## Этап 8. Деплой на Oracle Cloud ⏳ (ждёт capacity)
...
- Парсер abw.by — не начато
- Объединить все базы в одну
- Обновление базы, обновления отредактированных объявлений и удаление неактивных
...
*Последнее обновление: 2026-06-24. Добавлен av.by (этап 5), уточнена архитектура
мультиисточника и схема БД, UI описан по факту. Текущая задача — парсер abw.by.*